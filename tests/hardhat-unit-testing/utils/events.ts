...
export class InstanceEvents {
  ...
  print() {
    for (const log of this.events) {
      console.log(
        `LOG ${log.blockNumber}:${log.transactionIndex}:${log.logIndex} > ${
          log.event
        }(${JSON.stringify(log.returnValues)})`
      );
    }
  }
...
export function getEvents(instance: SmartContractInstance): InstanceEvents {
  ...
  if (!inst)
    throw new Error(
      'No Events subscribed for this instance. Did you call captureEvents before'
    );
  return inst;
}
